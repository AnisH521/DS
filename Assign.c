...
struct Node* Del_After(struct Node *Head)
{
	...
	if(wezp==NULL)
	{
		printf("\nNo Node exist after that for deletion\n");
		return Head;
	}
...
struct Node* Del_Before(struct Node *Head)
{
	...
	if(wezp3->data==value)
	{
		printf("\nNo Node exist before that for deletion\n");
		return Head;
	}

	else if(wezp3->link->data==value)
	{
		wezp3=wezp3->link;
		Head=wezp3;
		free(wezp);
