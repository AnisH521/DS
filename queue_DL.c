...
M.Sc Data Science
SEM 1
QUEUE IMPLEMENTATION OF DOUBLE LINKED LIST
*/
...
void enqueue() {
	...
	rear->rlink=newnode;//last node will now point to new node.
	newnode->llink=rear;//new node will also point to last node.
...
//This function will show the Elements in Queue.
void display() {
	nde temp=front;
	if(temp==NULL)
		printf("\nQueue is empty\n");
